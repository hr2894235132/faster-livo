...
void PointCloudPreprocess::AviaHandler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    cloud_out_.clear();
    cloud_full_.clear();
    uint plsize = msg->point_num; // 点云大小
    uint effect_ind = 0;

    cloud_out_.reserve(plsize);
    cloud_full_.resize(plsize);

    std::vector<bool> is_valid_pt(plsize, false); // 点云有效性标签
    std::vector<uint> index(plsize - 1); // 点云idx
//    for (uint i = 0; i < plsize - 1; ++i) {
//        index[i] = i + 1;  // 从1开始
//    }
//
//    // 并行化实现for循环
//    /* std::execution::seq:算法在单个线程中以确定性顺序执行，即不并行且不并发。
...
//    }
    for(uint i=1; i<plsize; i++)
    {
        ...
        if(effect_ind % point_filter_num_ == 0)
        {
            cloud_full_[i].x = msg->points[i].x;
            cloud_full_[i].y = msg->points[i].y;
            cloud_full_[i].z = msg->points[i].z;
            cloud_full_[i].intensity = msg->points[i].reflectivity;
            cloud_full_[i].curvature = msg->points[i].offset_time / float(1000000); //use curvature as time of each laser points
            cloud_out_.points.push_back(cloud_full_[i]);
        }
    }
}
