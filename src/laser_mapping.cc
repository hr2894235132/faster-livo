...
namespace faster_lio {
    ...
    bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
        ...
        // 0216 自适应体素结构参数 mapping algorithm params
        // noise model params
        ...
        nh.param<vector<int>>("voxel/layer_point_size", layer_point_size, vector<int>()); // 0216原代码中使用double
...
//        downSizeFilterMap.setLeafSize(filter_size_map_min_, filter_size_map_min_, filter_size_map_min_);
        ...
        // 12.8 初始化lidar_selection和imu的一些参数
...
//        p_imu_->SetAccBiasCov(V3D(0.001, 0.001, 0.001));
        return true;
    }
...
//        nh.param<string>("Result/result_path", result_path, "");
        ...
        // TODO 2023 12.27 待解决R3live数据集图像数据的问题
        ...
        pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered_only_pointCloud", 100);
    ...
    void LaserMapping::Run() {
        ...
        if (flg_EKF_inited_ && !init_map) {
            ...
            for (size_t i = 0; i < world_lidar_->size(); ++i) {
                ...
                point_this += lidar_T_wrt_IMU; // 注意外参矩阵要为旋转矩阵为单位阵的形式
                M3D point_crossmat;
...
//                                (-point_crossmat).transpose() + state.cov.block<3, 3>(3, 3);
                M3D cov_world = state.rot_end * cov_lidar * state.rot_end.transpose() +
                                ...
                                (-point_crossmat).transpose() * state.rot_end.transpose() + state.cov.block<3, 3>(3, 3);
                pv.cov = cov_world;
...
//                            PublishOdometry(pub_odom_aft_mapped_);
 ...
 << " " << state_point_.bg.transpose() << " " << state_point_.ba.transpose() << " ! " << state_point_.grav << std::endl;
        ...
        std::vector<M3D> body_var; // 存lidar系下点的cov
        std::vector<M3D> crossmat_list; // 存lidar系下的反对称矩阵
        /** lidar IEKF **/
        Timer::Evaluate(
                [&, this]() {
                    if (lidar_en) {
...
//                                                (-point_crossmat) * rot_var * (-point_crossmat).transpose() + t_var;
                                M3D cov_world = state.rot_end * cov_lidar * state.rot_end.transpose() +
                                                state.rot_end * (-point_crossmat) * rot_var *
                                                (-point_crossmat).transpose() * state.rot_end.transpose() + t_var;
                                pv.cov = cov_world;
                            ...
                            effect_feat_num_ = ptpl_list.size();
//                            effect_feat_num_ = 0;
...
//                                effect_feat_num_++;
                            ...
                            // 11.20 res_mean_last出现异常值 -nan，effect_feat_num_为0（已解决）
...
//                corr_norm_.resize(effect_feat_num_);
                            ...
                            if (effect_feat_num_ < 1) {
                                ...
                                continue; // return? or continue or break
                            }
                            // debug:
...
//                            solve_start = omp_get_wtime(); // 迭代求解开始
                            ...
                            // hr 0215更新IESKF迭代求解过程 在迭代过程中引入平面和点的不确定性
                            ...
                            for (int i = 0; i < effect_feat_num_; i++) {
                                ...
                                R_inv(i) = 1.0 / (sigma_l + norm_vec.transpose() * cov_lidar * norm_vec);
                                ...
                                laserCloudOri->points[i].normal_z =
                                        sqrt(norm_vec.transpose() * cov_lidar * norm_vec);
                                laserCloudOri->points[i].curvature =
                                        sqrt(sigma_l + norm_vec.transpose() * cov_lidar * norm_vec);
                                ...
                                Hsub_T_R_inv.col(i) << A[0] * R_inv(i), A[1] * R_inv(i),
                                        ...
                                        norm_p.z * R_inv(i); // 新公式 引入平面概率模型后的Hsub_T
#else
...
//                                MD(DIM_STATE, DIM_STATE) &&K_1 = (H_T_H + (state.cov).inverse()).inverse(); // 新卡尔曼增益公式前半部分
                                ...
                                MD(DIM_STATE, DIM_STATE) &&K_1 = (H_T_H + (state.cov /
                                                                           LASER_POINT_COV).inverse()).inverse();
                                cout << "!!!!!!!!!! " << LASER_POINT_COV << endl;// 新卡尔曼增益公式前半部分
//                                MD(DIM_STATE, DIM_STATE) &&K_1 = (H_T_H +
//                                                                  (state.cov).inverse()).inverse(); // 新卡尔曼增益公式前半部分
                ...
                [&, this]() {
                    ...
                    if (!time_sync_en_ && abs(last_timestamp_imu_ - last_timestamp_lidar_) > 10.0 &&
                        ...
                        LOG(INFO) << "IMU and LiDAR not Synced, IMU time: " << last_timestamp_imu_
                                  << ", lidar header time: " << last_timestamp_lidar_ << ", time diff: "
                                  << last_timestamp_imu_ - last_timestamp_lidar_ << endl;
                    }
                    if (time_sync_en_ && !timediff_set_flg_ && abs(last_timestamp_lidar_ - last_timestamp_imu_) > 1 &&
                        ...
                        LOG(INFO) << "Self sync IMU and LiDAR, time diff is " << timediff_lidar_wrt_imu_ << endl;
                    }
...
//        mtx_buffer_.lock();
        ...
        // hr imu异常数据去除
        if (abs(msg->angular_velocity.x) > 10 || abs(msg->angular_velocity.y) > 10 ||
            abs(msg->angular_velocity.z) > 10) {
            ROS_WARN("imu angular velocity abnormal, drop it!!! %.3f, %.3f, %.3f", msg->angular_velocity.x,
...
//        ROS_INFO("In sync");
        ...
        if (!lidar_pushed_) { // If not in lidar scan, need to generate new meas
            ...
            meas.lidar_beg_time_ = time_buffer_.front();
            ...
            if (meas.lidar_->points.size() <= 1) {
                ...
                sig_buffer_.notify_all();
                // ROS_ERROR("out sync");
                lidar_end_time_ = meas.lidar_beg_time_ + lidar_mean_scantime_;
                return false;
            } else if (meas.lidar_->points.back().curvature / double(1000) < 0.5 * lidar_mean_scantime_) {
                lidar_end_time_ = meas.lidar_beg_time_ + lidar_mean_scantime_;
            } else {
                // sort by sample timestamp; small to big
//                sort(meas.lidar_->points.begin(), meas.lidar_->points.end(), time_list);
                scan_num_++;
                lidar_end_time_ = meas.lidar_beg_time_ + meas.lidar_->points.back().curvature / double(1000);
                lidar_mean_scantime_ +=
                        (meas.lidar_->points.back().curvature / double(1000) - lidar_mean_scantime_) / scan_num_;
            }
            // sort by sample timestamp; small to big
            sort(meas.lidar_->points.begin(), meas.lidar_->points.end(), time_list);
...
//            pointWithCov pv;
//            pv.point << scan_down_body_->points[i].x, scan_down_body_->points[i].y, scan_down_body_->points[i].z;
//            // 计算lidar点的cov

//            // 注意这个在每次迭代时是存在重复计算的 因为lidar系的点云covariance是不变的
//            // M3D cov_lidar = calcBodyCov(pv.point, ranging_cov, angle_cov);
//            M3D cov_lidar = body_var[i];
//            M3D cov_world = transformLidarCovToWorld(pv.point, cov_lidar, state);
//            pv.cov = cov_world;
//            pv.point << world_lidar->points[i].x, world_lidar->points[i].y, world_lidar->points[i].z;
//            pv_list.push_back(pv);
//        }
...
// 0403 将计算世界坐标系下点的协方差模块化
    void
    LaserMapping::calcPointcov(pcl::PointCloud<pcl::PointXYZINormal>::Ptr &world_lidar_, const CloudPtr &scan_undistort,
                               std::vector<pointWithCov> pv_list) {
        for (size_t i = 0; i < world_lidar_->size(); ++i) {
            pointWithCov pv;
            pv.point << world_lidar_->points[i].x, world_lidar_->points[i].y, world_lidar_->points[i].z;
            V3D point_this(scan_undistort->points[i].x, scan_undistort->points[i].y, scan_undistort->points[i].z);
            // if z=0, error will occur in calcBodyCov. To be solved
            if (point_this[2] == 0) {
                point_this[2] = 0.001;
            }
...
//                  state.cov.block<3, 3>(3, 3);
            // hr 10.31
//            lidar_R_to_world = state.rot_end * lidar_R_wrt_IMU;
//            cov = lidar_R_to_world * cov * lidar_R_to_world.transpose() +
//                  lidar_R_to_world * (-point_crossmat) * state.cov.block<3, 3>(0, 0) *
//                  (-point_crossmat).transpose() * lidar_R_to_world.transpose() + state.cov.block<3, 3>(3, 3);
//            M3D cov_lidar;
//            calcBodyCov(point_this, ranging_cov, angle_cov, cov_lidar);
//            // hr 11.20
...
//            sigma_pv[2] = sqrt(sigma_pv[2]);
    ...
    // hr 11.6 修改并封装转换函数
    M3D LaserMapping::transformLidarCovToWorld(Eigen::Vector3d &point_lidar, const Eigen::Matrix3d &cov_lidar,
                                               const StatesGroup &_state) {
        M3D point_crossmat;
        point_crossmat << SKEW_SYM_MATRIX(point_lidar);

        /* lidar -> body */
        // 注意外参的var是先rot 后pos
        // FIXME: lidar_R_to_IMU.conjugate()???  (6, 6) (9, 9)
//    M3D cov_body = lidar_R_to_IMU * cov_lidar * lidar_R_to_IMU.transpose() +
//                   lidar_R_to_IMU * (-point_crossmat) * state.cov.block<3, 3>(6, 6) *
//                   (-point_crossmat).transpose() * lidar_R_to_IMU.transpose() + state.cov.block<3, 3>(9, 9);
        M3D cov_body = lidar_R_wrt_IMU * cov_lidar * lidar_R_wrt_IMU.transpose() +
                       lidar_R_wrt_IMU * (-point_crossmat) * _state.cov.block<3, 3>(0, 0) *
                       (-point_crossmat).transpose() * lidar_R_wrt_IMU.transpose() + _state.cov.block<3, 3>(3, 3);

        V3D point_body = lidar_R_wrt_IMU * point_lidar + lidar_T_wrt_IMU;
        /* body -> world */
        // 注意pose的var是先pos 后rot
        point_crossmat << SKEW_SYM_MATRIX(point_body);
        // state.rot_end.conjugate()
        M3D cov_world = _state.rot_end * cov_body * _state.rot_end.transpose() +
                        _state.rot_end * (-point_crossmat) * _state.cov.block<3, 3>(0, 0) *
                        (-point_crossmat).transpose() * _state.rot_end.transpose() + _state.cov.block<3, 3>(3, 3);
        return cov_world;
    }
}  // namespace faster_lio